...
void GameScene::removeBlock(list<int> blockTags, kBlock blockType)
{
    ...
    while(it != blockTags.end())
    {
        ...
        {
            // コマが消えるアニメーションを生成
            CCScaleTo* scale = CCScaleTo::create(REMOVEING_TIME, 0);
            
            // コマを削除するアクションを生成
            CCCallFuncN* func = CCCallFuncN::create(this, callfuncN_selector(GameScene::removeingBlock));
...
// コマの削除
void GameScene::removeingBlock(CCNode* block)
{
...
void GameScene::showLabel()
{
    ...
    while(it != blockTypes.end())
    {
        ...
        const char* countStr = ccsf("%2d", count);
